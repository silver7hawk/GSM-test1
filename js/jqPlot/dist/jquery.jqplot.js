...
(function ($) {
    ...
    */
    ...
    Series.prototype.init = function (index, gridbw, plot) {
        ...
        if (this.fillAlpha) {
            ...
            var colors = this.fillColor.split('|');
            if (colors.length > 1) {
                var comp = $.jqplot.normalize2rgb(colors[0]);
                var comp = $.jqplot.getColorComponents(comp);
                colors[0] = 'rgba(' + comp[0] + ',' + comp[1] + ',' + comp[2] + ',' + this.fillAlpha + ')';
                var comp = $.jqplot.normalize2rgb(colors[1]);
                var comp = $.jqplot.getColorComponents(comp);
                colors[1] = 'rgba(' + comp[0] + ',' + comp[1] + ',' + comp[2] + ',' + this.fillAlpha + ')';
                this.fillColor = colors[0] + '|' + colors[1];
            }
            else {
    ...
    // gridData - optional grid data point array to draw using this series renderer
    // stackData - array of cumulative data for stacked plots.
    Series.prototype.drawSingleBar = function (sctx, opts, plot) {
        var options = (opts == undefined) ? {} : opts;
        sctx = (sctx == undefined) ? this.canvas._ctx : sctx;
        ...
        if (this.show) {
            ...
            this.renderer.drawSingleBar.call(this, sctx, gridData, options, plot);
        }
    ...
    */
    function jqPlot() {
        ...
        this.draw = function () {
            if (this.drawIfHidden || this.target.is(':visible')) {
                ...
                if (this.options.rightPadding)
                    this._gridPadding.right = this.options.rightPadding;
        ...
        this.drawSeries = function (options, idx, drawSingleBar) {
            var i, series, ctx;
            ...
            if (idx != undefined) {
                ...
                if (drawSingleBar) {
                    series.drawSingleBar(ctx, options, this);
                }
                else {
                    ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    ...
    $.jqplot.CanvasGridRenderer.prototype.draw = function () {
        ...
        var bgColor = this.backgroundColor || this.background;
        var color = bgColor.split('|');
        var lingrad = ctx.createLinearGradient(0, 0, 0, 150);
        lingrad.addColorStop(0, color[0]);
        lingrad.addColorStop(1, color[1] != undefined ? color[1].toString() : color[0].toString());

        ctx.fillStyle = lingrad; //this.backgroundColor || this.background;
    ...
    // called within scope of series.
    $.jqplot.LineRenderer.prototype.drawSingleBar = function (ctx, gd, options, plot) {
        var i;
        // get a copy of the options, so we don't modify the original object.
        var opts = $.extend(true, {}, options);
        var shadow = (opts.shadow != undefined) ? opts.shadow : this.shadow;
    ...
    $.jqplot.LinearAxisRenderer.prototype.createTicks = function (plot) {
        ...
        // we don't have any ticks yet, let's make some!
            ...
            for (var i = 0; i < this.numberTicks; i++) {
                ...
                if (i == 0 && this.tickOptions.showLabelFirstTick == false)
                    t.label = '';
                //------------------
                this._ticks.push(t);
    ...
    */
    ...
    $.jqplot.ShapeRenderer.prototype.draw = function (ctx, points, options) {
        ...
        var colors = this.fillStyle.split('|');
        if (colors.length > 1) {
            var lingrad = ctx.createLinearGradient(0, 0, 150, 0);
            lingrad.addColorStop(0, colors[0]);
            lingrad.addColorStop(1, colors[1] != undefined ? colors[1].toString() : colors[0].toString());
    ...
    $.jqplot.TableLegendRenderer.prototype.draw = function () {
        ...
        if (this.show) {
            ...
            if (this.background) {
                ss['background'] = this.background.split('|')[0];
            }
            if (this.border) {
